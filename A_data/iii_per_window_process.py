...
def per_window_process(df, name, x_frames, y_frames, revenue, loss):
    # read cache, if exist.
    if os.path.exists(processed_data / (name + '_processed.pkl')):
        return pd.read_pickle(processed_data / (name + '_processed.pkl')), name
    ...
    # append label_1
    label_1 = pd.Series(0, index=r_value.index)
    label_1[r_value > revenue] = 1 
    label_1[r_value < loss] = -1
    
    # append label_2 
    label_2 = pd.Series(0, index=r_value.index)
    label_2[(r_value < beta) & (r_value > alpha)] = 1
    label_2[(r_value > -beta) & (r_value < -alpha)] = -1
    
    # save data
    new_df = pd.DataFrame({'X': X, 'y': y, 'label_1': label_1, 'label_2': label_2})
    new_name = name + f'X:{x_frames}_y:{y_frames}_r:{revenue}_processed'
    
    new_df.to_pickle(processed_data / (name + '.pkl'))
...
if __name__ == "__main__" : 
    ...
    # first load    
    new_df, name = per_window_process(df, name, 8, 2, 0.01, -0.01)
    ...
    new_df, name = per_window_process(df, name, 8, 2, 0.01, -0.01)
    print(type(new_df['X'].iloc[0][0][0]))
